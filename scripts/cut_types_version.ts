...
const typesPubspec = join("packages", "goals_types", "pubspec.yaml");
const versionFile = join("packages", "goals_types", "lib", "src", "version.dart");
...
const new_package_name = `goals_types_${
    peggedMajorVersion < 10 ? "0" : ""
}${peggedMajorVersion}`;

const originalBranch = execSync(`git rev-parse --abbrev-ref HEAD`).toString()
    .trim();
...
async function publishPeggedVersion() {
    ...
    // push
    execSync(`git push -f --set-upstream origin ${new_package_name}`);
    ...
    // run pub publish
    const proc = exec(
        `cd packages/goals_types && fvm dart pub publish -f`,
        (err, stdout, stderr) => {
            if (err) {
                rejecter(err);
            } else {
...
async function updateCurrentCode() {
    ...
    await promises.writeFile(
        versionFile,
        `const TYPES_VERSION = ${peggedMajorVersion + 1};`,
    );
